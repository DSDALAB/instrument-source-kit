...
class ServiceThread(threading.Thread):
    ...
    def run(self):
        ...
        # Write SampleRate, Write SampleRate同時也把舊Buffer資料清空
        ...
        while True:
            ...
            # # Read Feature
            # RMS = np.array( client.read_holding_registers(REG_ACC_RMS, DATA_LEGNTH, unit=1).registers ) 
            # RMS = RMS / 1000    # Gravity, 9.8 m/s^2
            # Peak = np.array( client.read_holding_registers(REG_ACC_PEAK, DATA_LEGNTH, unit=1).registers )
            # Peak = Peak / 1000  # Gravity, 9.8 m/s^2
            # CrestFactor = np.array( client.read_holding_registers(REG_ACC_CF, DATA_LEGNTH, unit=1).registers ) 
            # CrestFactor = CrestFactor / 1000
            # # SKEW & KU 約每3秒算1次 
            # Skewness = np.array( client.read_holding_registers(REG_ACC_SKEW, DATA_LEGNTH, unit=1).registers ) 
            # Skewness = Skewness / 1000
            # Kurtosis = np.array( client.read_holding_registers(REG_ACC_KU, DATA_LEGNTH, unit=1).registers ) 
